...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  // Check for AI Gateway or direct API configuration
  ...
  } else if (!env.ANTHROPIC_API_KEY && !env.GEMINI_API_KEY && !env.GROQ_API_KEY) {
    // Need at least one LLM API key
    missing.push('ANTHROPIC_API_KEY or GEMINI_API_KEY or GROQ_API_KEY or AI_GATEWAY_API_KEY');
  }
